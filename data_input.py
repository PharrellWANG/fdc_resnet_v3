...
def build_input(dataset, data_path, batch_size, mode, block_size,
	...
	Args:
		...
		num_of_classes: ..
		is_resize: if yes, is_resize is equal to the orginal size, if no, is_resize is None.
	Returns:
	...
	if dataset == 'fdc':
		...
		if is_resize is None:
			record_defaults = [[1] for _ in range(block_size * block_size + 1)]
		else:
			record_defaults = [[1] for _ in range(is_resize * is_resize + 1)]
